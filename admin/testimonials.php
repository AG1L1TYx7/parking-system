...
if(isset($_REQUEST['action']))
	{
		if($_REQUEST['action']=='pub')
			{
				mysql_query("update tbl_testimonial set status=1 where id=".$_REQUEST['id']."");
				$msg="Publish Successfully";
			}
		if($_REQUEST['action']=='unpub')
			{
				mysql_query("update tbl_testimonial set status=0 where id=".$_REQUEST['id']."");
				$msg="Unpublish Successfully";
			}
		if($_REQUEST['action']=='delete')
			{
				mysql_query("delete from tbl_testimonial  where id=".$_REQUEST['id']."");
...
if($_GET['d']=='up')
	{
		$res=mysql_query("select max(sort1) as sort1 from tbl_testimonial where sort1<".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
		
		
		
		$res1=mysql_query("select * from tbl_testimonial where sort1=".$new_rec."");
		$row1=mysql_fetch_array($res1);
		$old_rec=$row1['id'];
		
		
		
		mysql_query("UPDATE tbl_testimonial SET sort1=".$order." WHERE id=".$old_rec."");
		mysql_query("UPDATE tbl_testimonial SET sort1=".$new_rec." WHERE id=".$id."");
	
	}
if($_GET['d']=='down')
	{
		$res=mysql_query("select min(sort1) as sort1 from tbl_testimonial where sort1>".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
	...
	foreach($_POST['all_delete'] as $delete_id)
		{
			...
			mysql_query("delete from tbl_testimonial  where id=".$delete_id."");
...
if(isset($_REQUEST['submit']))
	{
		...
		{
			$find_max_res=mysql_query("SELECT MAX(sort1) as sort1 FROM tbl_testimonial");
			$find_max_count=mysql_num_rows($find_max_res);
			 if($find_max_count!=0)
				...
				$query=mysql_query("insert into tbl_testimonial(sort1,date1,usrname,company,description,status) values ('$sort1','".time()."','$name','$company','$description',1)");
		...
		// Edit page code start
			if($_POST['file_action']=='update')
			{
				...
				$query=mysql_query("update tbl_testimonial set 
				usrname='$name',
				company='$company',
...
$total_record = mysql_num_rows(mysql_query("select * from tbl_testimonial"));
...
$sql = "SELECT * FROM tbl_testimonial  ORDER BY sort1 ASC " . $pagination->getLimitSql();
$query=mysql_query($sql);
$order_row=mysql_num_rows($query);

if(isset($_GET['act']))
{
$edit_page = mysql_fetch_array(mysql_query("SELECT * FROM tbl_testimonial where id='".mysql_real_escape_string($_GET['id'])."'"));
}
 ?>
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<ul class="content-box-tabs">
						...
						<li><a href="#tab2"   class="default-tab">Modify</a></li>
						<?php } else { ?>
						  ...
						  <?php
						   ...
						   while($row=mysql_fetch_array($query)){ ?>		
								<tr>
									...
									<td valign="middle"><?php echo $row['usrname']; ?></td>
									<td><?php echo substr(stripslashes(urldecode($row['description'])),0,40); ?></td>
					...
					<div class="tab-content <?php if(isset($_GET['act'])) { ?> default-tab <?php } ?>" id="tab2">
						...
						<form action="#" method="post" enctype="multipart/form-data">
							...
							<fieldset> <!-- Set class to "column-left" or "column-right" on fieldsets to divide the form into columns -->
								...
								<p>
									<label>Name</label>
										<input class="text-input small-input" type="text" id="name" name="name"  value="<?php if(isset($_GET['act']))
{ echo $edit_page['usrname']; } else { echo ""; } ?>" />
								</p>
								<p>
									<label>Company Name</label>
										<input class="text-input small-input" type="text" id="company" name="company"  value="<?php if(isset($_GET['act']))
{ echo $edit_page['company']; } else { echo ""; } ?>" />
								</p>
