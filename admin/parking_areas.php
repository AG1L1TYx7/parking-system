...
if(isset($_REQUEST['action']))
	{
		if($_REQUEST['action']=='pub')
			{
				mysql_query("update tbl_parkingareas set status=1 where id=".$_REQUEST['id']."");
				$msg="Publish Successfully";
			}
		if($_REQUEST['action']=='unpub')
			{
				mysql_query("update tbl_parkingareas set status=0 where id=".$_REQUEST['id']."");
				$msg="Unpublish Successfully";
			}
		if($_REQUEST['action']=='delete')
			{
				mysql_query("delete from tbl_parkingareas  where id=".$_REQUEST['id']."");
...
if($_GET['d']=='up')
	{
		$res=mysql_query("select max(sort1) as sort1 from tbl_parkingareas where sort1<".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
		
		
		
		$res1=mysql_query("select * from tbl_parkingareas where sort1=".$new_rec."");
		$row1=mysql_fetch_array($res1);
		$old_rec=$row1['id'];
		
		
		
		mysql_query("UPDATE tbl_parkingareas SET sort1=".$order." WHERE id=".$old_rec."");
		mysql_query("UPDATE tbl_parkingareas SET sort1=".$new_rec." WHERE id=".$id."");
	
	}
if($_GET['d']=='down')
	{
		$res=mysql_query("select min(sort1) as sort1 from tbl_parkingareas where sort1>".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
	...
	foreach($_POST['all_delete'] as $delete_id)
		{
			...
			mysql_query("delete from tbl_parkingareas  where id=".$delete_id."");
...
if(isset($_REQUEST['submit']))
	{
		...
		$upload_dir="uploads/parkingareas/";
		
		
		// Add new page code start
		if($_POST['file_action']=='new')
		{
			$find_max_res=mysql_query("SELECT MAX(sort1) as sort1 FROM tbl_parkingareas");
	...
	$file_name="";
				...
				$query=mysql_query("insert into tbl_parkingareas(sort1,title,image,short_des,description,status) values ('$sort1','$title','$file_name','$short_des','$description',1)");
	...
	else
	{
	   $file_name=$_POST['image_name'];
				...
				$query=mysql_query("update tbl_parkingareas set 
				title='$title',
				short_des='$short_des',
				image='$file_name',
...
$total_record = mysql_num_rows(mysql_query("select * from tbl_parkingareas"));
...
$sql = "SELECT * FROM tbl_parkingareas  ORDER BY sort1 ASC " . $pagination->getLimitSql();
$query=mysql_query($sql);
$order_row=mysql_num_rows($query);

if(isset($_GET['act']))
{
$edit_page = mysql_fetch_array(mysql_query("SELECT * FROM tbl_parkingareas where id='".mysql_real_escape_string($_GET['id'])."'"));
}
 ?>
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<ul class="content-box-tabs">
						...
						<li><a href="#tab2"   class="default-tab">Modify Parking Areas </a></li>
						<?php } else { ?>
						<li><a href="#tab2">Add Parking Areas </a></li>
						<?php } ?>
					</ul>
						  ...
						  <?php
						   ...
						   while($row=mysql_fetch_array($query)){ ?>		
								<tr>
									...
									<td><img src="uploads/parkingareas/<?php echo $row['image']; ?>" width="150" height="100"></td>
...
{ echo $edit_page['title']; } else { echo ""; } ?>" />
                                	...
                                	<p>
									<label>Upload Image:(image  width="600" height="400")</label>
								...
								{?>
								<img src="uploads/parkingareas/<?php echo $edit_page['image']; ?>" width="150" height="100">
								<?php } else {} ?>	
