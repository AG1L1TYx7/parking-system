...
if(isset($_REQUEST['action']))
	{
		if($_REQUEST['action']=='pub')
			{
				mysql_query("update  tbl_contact_us set status=1 where id=".$_REQUEST['id']."");
				$msg="Publish Successfully";
			}
		if($_REQUEST['action']=='unpub')
			{
				mysql_query("update  tbl_contact_us set status=0 where id=".$_REQUEST['id']."");
				$msg="Unpublish Successfully";
			}
		if($_REQUEST['action']=='delete')
			{
				mysql_query("delete from  tbl_contact_us  where id=".$_REQUEST['id']."");
...
if($_GET['d']=='up')
	{
		$res=mysql_query("select max(sort1) as sort1 from  tbl_contact_us where sort1<".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
		
		
		
		$res1=mysql_query("select * from  tbl_contact_us where sort1=".$new_rec."");
		$row1=mysql_fetch_array($res1);
		$old_rec=$row1['id'];
		
		
		
		mysql_query("UPDATE  tbl_contact_us SET sort1=".$order." WHERE id=".$old_rec."");
		mysql_query("UPDATE  tbl_contact_us SET sort1=".$new_rec." WHERE id=".$id."");
	
	}
if($_GET['d']=='down')
	{
		$res=mysql_query("select min(sort1) as sort1 from  tbl_contact_us where sort1>".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
	...
	foreach($_POST['all_delete'] as $delete_id)
		{
			...
			mysql_query("delete from  tbl_contact_us  where id=".$delete_id."");
...
if(isset($_REQUEST['submit']))
	{
		 ...
		 $address=addslashes(urlencode($_POST['address']));
		...
		{
			$find_max_res=mysql_query("SELECT MAX(sort1) as sort1 FROM  tbl_contact_us");
	...
	$query=mysql_query("insert into tbl_contact_us(sort1,email,phone,fax,address,status) values ('$sort1','$email','$phone','$fax','$address',1)");
		...
		if($_POST['file_action']=='update')
			{
				...
				$query=mysql_query("update  tbl_contact_us set 
				email='$email',
				phone='$phone',
				fax='$fax',
				address='$address'
...
$total_record = mysql_num_rows(mysql_query("select * from  tbl_contact_us"));
...
$sql = "SELECT * FROM  tbl_contact_us  ORDER BY sort1 ASC " . $pagination->getLimitSql();
$query=mysql_query($sql);
$order_row=mysql_num_rows($query);

if(isset($_GET['act']))
{
$edit_page = mysql_fetch_array(mysql_query("SELECT * FROM  tbl_contact_us where id='".mysql_real_escape_string($_GET['id'])."'"));
}
 ?>
            ...
            <table>
              <thead>
                <tr>
                  ...
                  <th width="732"> email </th>
				  <th width="732"> Phone NO </th>
				  <th width="732"> Fax NO </th>
                  <th width="52">Sort</th>
        ...
        <div class="tab-content <?php if(isset($_GET['act'])) { ?> default-tab <?php } ?>" id="tab2">
          <form action="#" method="post" name="form1"  enctype="multipart/form-data">
            ...
            <!-- Set class to "column-left" or "column-right" on fieldsets to divide the form into columns -->
				...
				<label>Email:</label>
									<input class="text-input medium-input" type="text" name="email" id="email"  value="<?php if(isset($_GET['act']))
{ echo $edit_page['email']; } else { echo ""; } ?>" />
			...
			<label>Phone No:</label>
									...
									<input class="text-input medium-input" type="text" name="phone" id="phone"  value="<?php if(isset($_GET['act']))
{ echo $edit_page['phone']; } else { echo ""; } ?>" />
								</p>
         
								
            <!--<p>
			<label>Fax No:</label>
									<input class="text-input medium-input" type="text" name="fax" id="fax"  value="<?php //if(isset($_GET['act']))
//{ echo $edit_page['fax']; } else { echo ""; } ?>" />
								</p>-->
            	...
            	<p>
						...
						<div><textarea id="address" name="address"><?php if(isset($_GET['act']))
{ echo stripslashes(urldecode($edit_page['address'])); } else { echo ""; } ?></textarea>
			<script type="text/javascript">
//<![CDATA[
