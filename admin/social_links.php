...
if(isset($_REQUEST['action']))
	{
		if($_REQUEST['action']=='pub')
			{
				mysql_query("update tbl_social_link set status=1 where id=".$_REQUEST['id']."");
				$msg="Publish Successfully";
			}
		if($_REQUEST['action']=='unpub')
			{
				mysql_query("update tbl_social_link set status=0 where id=".$_REQUEST['id']."");
				$msg="Unpublish Successfully";
			}
		if($_REQUEST['action']=='delete')
			{
				mysql_query("delete from tbl_social_link  where id=".$_REQUEST['id']."");
...
if($_GET['d']=='up')
	{
		$res=mysql_query("select max(sort1) as sort1 from tbl_social_link where sort1<".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
		
		
		
		$res1=mysql_query("select * from tbl_social_link where sort1=".$new_rec."");
		$row1=mysql_fetch_array($res1);
		$old_rec=$row1['id'];
		
		
		
		mysql_query("UPDATE tbl_social_link SET sort1=".$order." WHERE id=".$old_rec."");
		mysql_query("UPDATE tbl_social_link SET sort1=".$new_rec." WHERE id=".$id."");
	
	}
if($_GET['d']=='down')
	{
		$res=mysql_query("select min(sort1) as sort1 from tbl_social_link where sort1>".$order."");
		$row=mysql_fetch_array($res);
		$new_rec=$row['sort1'];
	...
	foreach($_POST['all_delete'] as $delete_id)
		{
			...
			mysql_query("delete from tbl_social_link  where id=".$delete_id."");
...
if(isset($_REQUEST['submit']))
	{
		...
		$soical_link =  mysql_real_escape_string(str_replace("'","&#8217;", $_POST['soical_link']));
		
		$upload_dir="uploads/social_links/";
		
		
		// Add new page code start
		if($_POST['file_action']=='new')
		{
			$find_max_res=mysql_query("SELECT MAX(sort1) as sort1 FROM tbl_social_link");
	...
	$file_name="";
				...
				$query=mysql_query("insert into tbl_social_link(sort1,title,image,soical_link,status) values ('$sort1','$title','$file_name','$soical_link',1)");
	...
	else
	{
	   $file_name=$_POST['image_name'];
				...
				$query=mysql_query("update tbl_social_link set 
				title='$title',
				soical_link='$soical_link',
				image='$file_name'
...
$total_record = mysql_num_rows(mysql_query("select * from tbl_social_link"));
...
$sql = "SELECT * FROM tbl_social_link  ORDER BY sort1 ASC " . $pagination->getLimitSql();
$query=mysql_query($sql);
$order_row=mysql_num_rows($query);

if(isset($_GET['act']))
{
$edit_page = mysql_fetch_array(mysql_query("SELECT * FROM tbl_social_link where id='".mysql_real_escape_string($_GET['id'])."'"));
}
 ?>
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<ul class="content-box-tabs">
						...
						<li><a href="#tab2"   class="default-tab">Modify Social Links </a></li>
						<?php } else { ?>
						<li><a href="#tab2">Add Social Links </a></li>
						<?php } ?>
					</ul>
						  ...
						  <?php
						   ...
						   while($row=mysql_fetch_array($query)){ ?>		
								<tr>
									...
									<td><img src="uploads/social_links/<?php echo $row['image']; ?>" width="25" height="25"></td>
...
{ echo $edit_page['title']; } else { echo ""; } ?>" />
                                	...
                                	<p>
									<label>Upload Image:(image width and height must be 450 x 230)</label>
								...
								{?>
								<img src="uploads/social_links/<?php echo $edit_page['image']; ?>" width="12" height="12">
								<?php } else {} ?>	
								...
								<p>
									<label>Link:</label>
									<input class="text-input large-input" type="text" id="metatags" name="soical_link"  value="<?php if(isset($_GET['act']))
{ echo $edit_page['soical_link']; } else { echo ""; } ?>" />
								</p>
