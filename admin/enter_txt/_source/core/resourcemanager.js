 ...
 * @param {String} basePath The path for the proj_resources folder.
...
{
	/**
	 * The base directory containing all proj_resources.
	...
	/**
	 * Contains references to all proj_resources that have already been registered
	 * with {@link #add}.
	...
	/**
	 * Contains references to all proj_resources that have already been loaded
	 * with {@link #load}.
...
CKEDITOR.resourceManager.prototype =
{
	...
	/**
	 * Registers one or more proj_resources to be loaded from an external path
	...
	/**
	 * Loads one or more proj_resources.
	 * @param {String|Array} name The name of the resource to load. It may be a
	 *		string with a single resource name, or an array with several names.
	 * @param {Function} callback A function to be called when all proj_resources
	...
	load : function( names, callback, scope )
	{
		...
		for ( var i = 0 ; i < names.length ; i++ )
		{
			...
			if ( !loaded[ name ] && !registered[ name ] )
			{
				...
				proj_resources[ name ] = this.get( name );
		}
		...
		CKEDITOR.scriptLoader.load( urls, function( completed, failed )
			{
				...
				callback.call( scope, proj_resources );
			}
			, this);
	}
};
