...

	{
		...
		if($_REQUEST['action']=='pub')

			{
				...
				mysql_query("update tbl_user set status=1 where id=".$_REQUEST['id']."");
		...
		if($_REQUEST['action']=='unpub')

			{
				...
				mysql_query("update tbl_user set status=0 where id=".$_REQUEST['id']."");
...
if($_GET['d']=='up')

	{
		...
		$res=mysql_query("select max(sort1) as sort1 from tbl_user where sort1<".$order."");
		...
		$res1=mysql_query("select * from tbl_user where sort1=".$new_rec."");
		...
		mysql_query("UPDATE tbl_user SET sort1=".$order." WHERE id=".$old_rec."");

		mysql_query("UPDATE tbl_user SET sort1=".$new_rec." WHERE id=".$id."");
...
if($_GET['d']=='down')

	{
		...
		$res=mysql_query("select min(sort1) as sort1 from tbl_user where sort1>".$order."");
...
if(isset($_POST['all_delete']))
	...
	foreach($_POST['all_delete'] as $delete_id)

		{
			...
			mysql_query("delete from tbl_user  where id=".$delete_id."");
...

		{
			...
			$find_max_count=mysql_num_rows($find_max_res);
			 ...
			 if($find_max_count!=0)
				...
				$query=mysql_query("insert into tbl_user(sort1,fname,lname,telephone,password,email,address,status) values ('$sort1','$fname','$lname','$telephone','$password','$email','$address',1)");
...
$sql = "SELECT * FROM tbl_user  ORDER BY sort1 ASC " . $pagination->getLimitSql();
...
$edit_page = mysql_fetch_array(mysql_query("SELECT * FROM tbl_user where id='".mysql_real_escape_string($_GET['id'])."'"));
		...
		<div id="main-content"> <!-- Main Content Section with everything -->
			...
			<div class="content-box"><!-- Start Content Box -->
				...
				<div class="content-box-header">
					...
					<h3>Manage Registerted Users</h3>
					...
					<ul class="content-box-tabs">
						...
						<li><a href="#tab2"   class="default-tab">Modify Registerted Users</a></li>
...
{ echo $edit_page['email']; } else { echo ""; } ?>" />
                                 ...
                                 <p>
									...
									<input class="text-input small-input" type="text" id="address" name="address"  value="<?php if(isset($_GET['act']))
{ echo $edit_page['address']; } else { echo ""; } ?>" />
